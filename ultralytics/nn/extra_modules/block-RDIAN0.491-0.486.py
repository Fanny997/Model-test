...
class GCAI_Fusion(nn.Module):
    def __init__(self, c_low, c_high, c_out):
        ...
        # 2. 高层特征增强 (仿照论文的 Attentional Mechanism)
        # 论文中对High-level特征使用了SA+CA (或者CBAM)，这里我们用一个轻量级的注意力来增强High
    ...
    def forward(self, x):
        # 解包：[x_low (P3/P2), x_high (Upsampled P4/P5)]
        x_low, x_high = x

        # 1. 尺寸对齐 (上采样 High 到 Low 的尺寸)
        ...
        # 3. 对 High 特征进行注意力增强 (Context Guidance)
        # 这一步是为了让 DRG 提取的高级语义更准确
        high_att = self.att_high(high_feat) * high_feat
        high_att = self.att_spatial(high_att) * high_att

        # 4. 加法融合 (Paper Strategy: High_Upsampled + Low)
        # 这种方式保证了底层的小目标细节（low_feat）不会被高层特征“门控”掉
...
class MDTE_Simplified(nn.Module):
    """
    简化版 MDTE，用于特征层而非原始图像
    """
    def __init__(self, channels):
        super().__init__()
        # 用差分卷积提取方向特征 (这里用普通卷积模拟方向差分)
